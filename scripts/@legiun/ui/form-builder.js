	...
	if (viewer === "self") {
		...
		v.body(
			[
				...
				`	§3» §f${Math.max(Math.floor(player.get("playtime") / 3600), 0)} §bHours`,
				`	§3» §f${Math.max(Math.floor(player.get("playtime") / 60), 0)} §bMinutes`,
				`	§3» §f${Math.max(Math.floor(player.get("playtime")), 0)} §bSeconds`
			].join("\n")
		);
	}
};
